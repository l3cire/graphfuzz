...
class MSTTesterAlgorithms:
    ...
    def _weight_sum(mst_edges):
        return sum(data['weight'] if 'weight' in data else 1 for _, _, data in list(mst_edges))
    
    @staticmethod
    def kruskal(graph: nx.DiGraph):
        return MSTTesterAlgorithms._weight_sum(nx.minimum_spanning_edges(graph, algorithm='kruskal', data=True))
    
    @staticmethod
    def prim(graph: nx.DiGraph):
        return MSTTesterAlgorithms._weight_sum(nx.minimum_spanning_edges(graph, algorithm='prim', data=True))
    
    @staticmethod
    def boruvka(graph: nx.DiGraph):
        return MSTTesterAlgorithms._weight_sum(nx.minimum_spanning_edges(graph, algorithm='boruvka', data=True))
    ...
    def igraph(graph: nx.DiGraph):
        ...
        return sum(edge['weight'] for edge in graph_ig.spanning_tree(weights='weight').es)
