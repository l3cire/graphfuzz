...
class HarmonicCentralityTesterAlgorithms:
    ...
    def igraph(graph: nx.DiGraph):
        ...
        if 'weight' in graph_ig.es.attribute_names():
            ig_centrality = graph_ig.harmonic_centrality(mode=mode, weights='weight', normalized=False)
        else:
        ...
        return {graph_ig.vs['name'][v.index]: c for v, c in zip(graph_ig.vs, ig_centrality)}
