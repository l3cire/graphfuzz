        ...
        if discrepancies:
            for discrepancy_message, graph in discrepancies.items():
                if discrepancy_message not in first_occurrence_times:
                    ...
                    print(
                        f"Recorded first occurrence of '{discrepancy_message}' at {first_occurrence_times[discrepancy_message]} seconds since start."
                    )
                    total_bug_counts[discrepancy_message] = (
                        total_bug_counts.get(discrepancy_message, 0) + 1
                    )
